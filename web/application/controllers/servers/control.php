...
class controlController extends Controller {
	...
	public function console($serverid = null) {
		$this->load->checkLicense();
		if(!$this->user->isLogged()) {
	  		$this->data['status'] = "error";
			$this->data['error'] = "Вы не авторизированы!";
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			$file = "/home/gs" . $serverid;
			
			$server = $this->serversModel->getServerById($serverid, array('games'));
			
			switch($server['game_code']) {
				case 'samp': $file .= "/server_log.txt"; break;
				case 'mine': $file .= "/logs/latest.log"; break;
			}
			
			$this->data['status'] = "success";
			$this->data['text'] = file_get_contents($file);
		}
	...
	private function validatePOST() {
		...
		if($editpassword) {
			...
			$result = "Невозможно сохранить пустые значения!";
		}
		return $result;
	}
}
